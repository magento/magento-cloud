...
abstract class QueueTestCaseAbstract extends \PHPUnit_Framework_TestCase
{
    ...
    protected function waitForAsynchronousResult($expectedLinesCount, $logFilePath)
    {
        ...
        } while (($expectedLinesCount !== $actualCount) && ($i++ < 100));

        if (!file_exists($logFilePath) || $expectedLinesCount !== $actualCount) {
            $this->fail("No or not all asynchronous messages were processed.");
        }
    }
