 ...
 * 3. Opens the Create System Report page
 * 4. Clicks on Create and generates Design report from the multiselect.
 * 5. Verifies the success message and the grid for the new report.
 * 6. Clicks on Create and generates Configuration report from the multiselect.
 * 7. Verifies the success message and the grid for the new report.
 * 8. After scenario completion deletes all reports by mass delete.
 */
class CreateReportEntityStep implements TestStepInterface
{
    ...
    /**
     * Report Design fixture.
     *
     * @var SupportReport
     */
    private $supportReportDesign;
     ...
     * @var SupportReport
     */
    private $supportReportConfiguration;
     ...
     * @param SupportReport $supportReportDesign
     * @param SupportReport $supportReportConfiguration
    ...
    public function __construct(
        ...
        SupportReport $supportReportDesign,
        SupportReport $supportReportConfiguration,
        ...
        $this->supportReportDesign = $supportReportDesign;
        $this->supportReportConfiguration = $supportReportConfiguration;
    ...
    public function run()
    {
        ...
        $reportsFixtureArray = [$this->supportReportDesign, $this->supportReportConfiguration];
        foreach ($reportsFixtureArray as $reportFixture) {
            $existingCount = count($this->reportIndex->getReportsGridBlock()->getAllIds());
            $this->reportIndex->getPageActionsBlock()->addNew();
            $this->reportNew->getReportForm()->fill($reportFixture);
